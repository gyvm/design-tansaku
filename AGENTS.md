...
## AI エージェント行動指針 (Agent Behavior Guidelines)

### 言語設定 (Language Preference)

- **Implementation Plan (実装計画)** および **Walkthrough (ウォークスルー)** は、必ず **日本語** で記述してください。
- コード内のコメントやコミットメッセージは、プロジェクトの既存の慣習（主に英語、または日本語）に従ってください。
- ユーザーとの対話は日本語で行ってください。

### アーティファクト作成ルール

1. **Implementation Plan (実装計画)**
   - ファイル名: `implementation_plan.md`
   - 言語: 日本語
   - 内容: 変更の目的、ユーザーレビューが必要な項目、変更内容の詳細、検証計画

2. **Walkthrough (ウォークスルー)**
   - ファイル名: `walkthrough.md`
   - 言語: 日本語
   - 内容: 実施した変更の概要、変更されたファイル、検証結果、スクリーンショット（あれば）
